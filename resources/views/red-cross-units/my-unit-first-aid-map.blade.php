<x-layouts.app title="First Aid Coverage">
    <x-slot name="styles">
        <link rel="stylesheet"
    ...
    <div class="min-h-screen py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-sm text-gray-600 mb-4">Bubble size = number of first aiders · Colour = training freshness (green = recent)</p>

            {{-- Level toggle --}}
            <div class="flex gap-2 mb-4">
                <a href="{{ route('red-cross-units.my-unit-first-aid-map', ['level' => 'branch']) }}"
                ...
                <a href="{{ route('red-cross-units.my-unit-first-aid-map', ['level' => 'division']) }}"
                   class="px-4 py-2 rounded text-sm font-medium
                          {{ $level === 'division'
                               ? 'bg-indigo-600 text-white'
                               : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
            ...
            @if (! $hasFreshness)
                <p class="mb-3 text-sm text-gray-600 bg-gray-50 border border-gray-200 rounded px-3 py-2">
                    ...
                    Freshness not yet computed. Bubbles shown in neutral colour.
                </p>
            @endif
    ...
    <x-slot name="scripts">
        ...
        <script>
            ...
            function heatColor(heat, count) {
                if (count === 0) return '#ffffff';                   // no first aiders → white
