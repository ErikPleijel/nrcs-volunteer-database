<x-layouts.app title="Volunteer Map">
    <x-slot name="styles">
        <link rel="stylesheet"
    ...
    <div class="min-h-screen py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-sm text-gray-600 mb-4">Bubble size = volunteer count · Color = activity intensity</p>

            {{-- Level toggle --}}
            <div class="flex gap-2 mb-4">
                <a href="{{ route('red-cross-units.my-unit-volunteer-map', ['level' => 'branch']) }}"
                ...
                <a href="{{ route('red-cross-units.my-unit-volunteer-map', ['level' => 'division']) }}"
                   class="px-4 py-2 rounded text-sm font-medium
                          {{ $level === 'division'
                               ? 'bg-indigo-600 text-white'
                               : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
        ...
        <script>
            ...
            sorted.forEach(p => {
                L.circleMarker([p.lat, p.lng], {
                    ...
                    color:       '#374151',
                    weight:      1,
                    fillColor:   heatColor(p.heat, p.count),
                    fillOpacity: 0.75,
                })
                .bindPopup(
                    `<strong>${p.name}</strong><br>` +
                    `${p.count.toLocaleString()} volunteers<br>` +
                    `Activity heat: ${p.heat === null ? 'not yet computed' : Math.round(p.heat * 100) + '%'}`
                )
                .addTo(map);
            });
            ...
            let html = '<p class="font-semibold text-gray-700 mb-1">Activity intensity</p>';
            html += `
                <div class="mb-1">
            ...
            // Gray swatch — only if any point has null heat
            if (points.some(p => p.heat === null)) {
                html += `
            ...
            // White swatch — only if any point has zero volunteers
