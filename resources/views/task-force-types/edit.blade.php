    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Task Force Type') }}: {{ $taskForceType->name }}
            </h2>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('task-force-types.update', $taskForceType) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text"
                                   name="name"
                                   id="name"
                                   value="{{ old('name', $taskForceType->name) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <select name="level"
                                ...
                                @for($i = 1; $i <= 10; $i++)
                                    <option value="{{ $i }}" {{ old('level', $taskForceType->level) == $i ? 'selected' : '' }}>
                                        Level {{ $i }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div class="mb-6">
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       id="include_in_list"
                                       value="1"
                                       {{ old('include_in_list', $taskForceType->include_in_list) ? 'checked' : '' }}
