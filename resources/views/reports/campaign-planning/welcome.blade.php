<x-layouts.admin title="Welcome Campaign Planner">
    <x-slot name="pageHeader">
        <i class="fas fa-envelope-open-text mr-3 mb-6"></i> Welcome Campaign Planner
    </x-slot>
    ...
    <div class="container mx-auto px-4 py-6">
                    ...
                    <div x-data="{ open: null }" class="space-y-1 text-sm mb-4">
                        ...
                        <div class="rounded-md border border-gray-200 overflow-hidden">
                            <button type="button"
                                ...
                                <span><i class="fas fa-envelope-open-text mr-2 text-indigo-400"></i>Plan Campaigns</span>
        ...
        <div class="filter-container">
            <div class="filter-form-content">
                <form action="{{ route('reports.campaign-planning.welcome') }}" method="GET" class="filter-form">
                    ...
                    <div class="filter-grid filter-grid-4">
                        ...
                        {{-- Col 1: Contribution — pill toggle --}}
                        <div x-data="{ contribution: '{{ request('contribution', 'volunteering') }}' }">
                            <label class="filter-label">Contribution preference</label>
                            ...
                            <div class="flex rounded-md overflow-hidden border border-gray-200 divide-x divide-gray-200">
                                <button type="button"
                                        @click="contribution = ''"
                                        :class="contribution === '' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                                        class="flex-1 px-3 py-1.5 text-sm font-medium transition-colors">
                                    Both
                                </button>
                                <button type="button"
                                        @click="contribution = 'volunteering'"
                                        :class="contribution === 'volunteering' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                                        class="flex-1 px-3 py-1.5 text-sm font-medium transition-colors">
                                    Volunteer
                                </button>
                                <button type="button"
                                        @click="contribution = 'member'"
                                        :class="contribution === 'member' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                                        class="flex-1 px-3 py-1.5 text-sm font-medium transition-colors">
                                    Member
                                </button>
                            </div>
                        ...
                        {{-- Col 2: Registered within --}}
                        <div>
                            <label for="registered_within" class="filter-label">Registered within</label>
                            <select name="registered_within" id="registered_within"
                                    class="filter-select {{ request('registered_within') ? 'filter-active' : '' }}">
                                <option value="">All time</option>
                                <option value="30"  @selected(request('registered_within') == '30')>Last 30 days</option>
                                <option value="60"  @selected(request('registered_within') == '60')>Last 60 days</option>
                                <option value="90"  @selected(request('registered_within') == '90')>Last 90 days</option>
                                <option value="180" @selected(request('registered_within') == '180')>Last 180 days</option>
                            </select>
                        </div>
                    ...
                    <div class="filter-actions">
                        <div class="filter-button-group">
                            ...
                            @if($isDivisionLevel)
                                <a href="{{ route('reports.campaign-planning.welcome', ['branch_id' => request('branch_id')]) }}"
                                   class="filter-btn-secondary filter-btn-secondary-active">
                                    <i class="fas fa-times mr-1"></i>Clear
                                </a>
                            @else
                                <a href="{{ route('reports.campaign-planning.welcome') }}"
                                   class="filter-btn-secondary filter-btn-secondary-active">
                                    <i class="fas fa-times mr-1"></i>Clear
                                </a>
                            @endif
        ...
        @if($isDivisionLevel)
            <div class="mt-4 text-sm text-gray-600 flex items-center gap-1">
                ...
                {{-- branch_id='' in the URL tells the controller the user navigated up intentionally,
                     preventing the auto-redirect from bouncing branch admins straight back. --}}
                <a href="{{ route('reports.campaign-planning.welcome', array_merge($filterParams, ['branch_id' => ''])) }}"
        ...
        <div class="bg-white rounded-lg shadow mt-4 overflow-x-auto">
            @if(empty($planningData))
                <div class="py-12 text-center text-gray-400 italic">No pending volunteers or members found for the current filters.</div>
            @else
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="text-center px-4 py-3 font-semibold text-gray-600">Total pending</th>
                            ...
                            <th class="text-center px-4 py-3 font-semibold text-gray-600">Avg days since<br>registered</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @php
                            $summaryAvgClass = $summaryAvgDays === null
                                ? 'text-gray-400'
                                : ($summaryAvgDays <= 30 ? 'text-green-600' : ($summaryAvgDays <= 90 ? 'text-amber-500' : 'text-red-600'));
                        @endphp
                        ...
                        @foreach($planningData as $row)
                            @php
                                ...
                                $avgClass = $avgDays === null
                                    ? 'text-gray-400'
                                    : ($avgDays <= 30 ? 'text-green-600' : ($avgDays <= 90 ? 'text-amber-500' : 'text-red-600'));
