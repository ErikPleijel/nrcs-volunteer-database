...
test('editing an approved payment resets it to pending and clears the previous decision', function () {
    $submitter = User::factory()->inBranch($this->branchA)->create();
    $submitter->assignRole('branch_db_assistant');
    $approver = User::factory()->inBranch($this->branchA)->create();
    $approver->assignRole('branch_secretary');
    ...
    $this->actingAs($approver)
        ->put(route('membership-payments.update', $payment->id), [
            'user_id' => $member->id,
            'membership_fee_id' => $fee->id,
        ...
        ->assertRedirect(route('membership-payments.show', $payment->id));
...
test('resetting an approved payment to pending recomputes the member\'s lifecycle status', function () {
    $submitter = User::factory()->inBranch($this->branchA)->create();
    $submitter->assignRole('branch_db_assistant');
    $approver = User::factory()->inBranch($this->branchA)->create();
    $approver->assignRole('branch_secretary');
    // No red_cross_unit_id and no assigned_rcu_date, so this member is policy-typed
    // 'member' (not 'volunteer'/'unassigned'): dormancy is governed purely by
    // whether they hold a current valid membership payment.
    $member = User::factory()->inBranch($this->branchA)->create([
        'lifecycle_status' => 'active',
    ]);
    $fee = MembershipFee::factory()->create(['validity_years' => 1]);
    ...
    expect($member->currentMembershipPayment()->personal()->exists())->toBeTrue()
        ->and($member->refresh()->lifecycle_status)->toBe('active');

    // Editing the (only) approved payment demotes it back to pending, so it no
    // longer counts as a current valid membership under the ApprovedScope —
    // resetApprovalOnEdit()'s unconditional recalculateLifecycle() call should
    // observe that and demote the member to dormant.
    $this->actingAs($approver)
        ->put(route('membership-payments.update', $payment->id), [
            'user_id' => $member->id,
            'membership_fee_id' => $fee->id,
            'payment_date' => $payment->payment_date->toDateString(),
            'expiry_date' => $payment->expiry_date->toDateString(),
...
/*
|--------------------------------------------------------------------------
| PART 3 — module-specific: expiry_date derivation
|--------------------------------------------------------------------------
*/
