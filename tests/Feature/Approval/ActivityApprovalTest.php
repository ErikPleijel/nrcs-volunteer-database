...
test('editing an approved activity resets it to pending and clears the previous decision', function () {
    $submitter = User::factory()->inBranch($this->branchA)->create();
    $submitter->assignRole('branch_db_assistant');
    $approver = User::factory()->inBranch($this->branchA)->create();
    $approver->assignRole('branch_secretary');
    $member = User::factory()->inBranch($this->branchA)->create();
    $activityType = ActivityType::create(['name' => 'General Volunteering']);
...
test('resetting an approved activity to pending recomputes the member\'s lifecycle status', function () {
    $submitter = User::factory()->inBranch($this->branchA)->create();
    $submitter->assignRole('branch_db_assistant');
    $approver = User::factory()->inBranch($this->branchA)->create();
    $approver->assignRole('branch_secretary');
    ...
    // solely from this activity once approved (no other activity/payment/training/
    // donation exists for them in this test).
    $member = User::factory()->inBranch($this->branchA)->create([
    ...
    expect($member->refresh()->lifecycle_status)->toBe('active')
        ->and($member->last_activity_at?->toDateString())->toBe($activity->date->toDateString());
...
/*
|--------------------------------------------------------------------------
| PART 3 — module-specific: the polymorphic assignable pair
|--------------------------------------------------------------------------
*/
...
test('an activity with no assignable unit can still go through the full approve and demote cycle without error', function () {
    $submitter = User::factory()->inBranch($this->branchA)->create();
    $submitter->assignRole('branch_db_assistant');
    $approver = User::factory()->inBranch($this->branchA)->create();
    $approver->assignRole('branch_secretary');
    ...
    expect($activity->assignable_id)->toBeNull()
        ->and($activity->assignable_type)->toBeNull();
    ...
    $fresh = freshActivity($activity->id);
    expect($fresh->approval_status)->toBe(Activity::APPROVED)
        ->and($fresh->assignable_id)->toBeNull()
        ->and($fresh->assignable_type)->toBeNull();

    // Still no assignable unit — editing it demotes back to pending without error.
    $this->actingAs($approver)
        ->put(route('activities.update', $activity->id), [
            'activity_type_id' => $activityType->id,
            'user_id' => $member->id,
